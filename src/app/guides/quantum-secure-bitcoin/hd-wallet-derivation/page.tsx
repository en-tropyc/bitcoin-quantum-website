...
export default function HDWalletDerivationGuide() {
  return (
    <GuideLayout
      ...
      slug="/guides/quantum-secure-bitcoin/hd-wallet-derivation"
      datePublished="2026-08-19"
    >
      <div className="guide-note">
        <span className="guide-note-label">Update &middot; v0.4.1-testnet</span>
        <p>
          This guide analyzes BTQ <strong>v0.3.0-testnet</strong>, whose Dilithium HD derivation was an
          unfinished placeholder. The codebase has since moved on &mdash; read the sections below as a record
          of that snapshot and the reasoning behind it. Two things are now different:
        </p>
        <p>
          <strong>Hardened derivation works.</strong> As of v0.4.1-testnet the compiled wallet implements
          Construction&nbsp;1 (below): <code>CDilithiumExtKey::Derive()</code> HMAC-expands a per-child seed
          and re-runs ML-DSA key generation, producing valid keys. Seed-based recovery works &mdash; a single
          wallet HD seed regenerates every Dilithium key, over hardened-only paths that mirror{' '}
          <code>m/0&rsquo;/0&rsquo;/n&rsquo;</code>.
        </p>
        <p>
          <strong>Watch-only derivation still does not.</strong> Non-hardened / extended-public-key derivation
          remains unimplemented (<code>CDilithiumExtPubKey::Derive()</code> returns <code>false</code>), which
          is the genuinely open problem this guide is about.
        </p>
        <p>
          One correction to the analysis below: the compiled HD-key type has always been{' '}
          <code>CDilithiumExtKey</code> in{' '}
          <a
            href="https://github.com/btq-ag/btq-core/blob/v0.4.1-testnet/src/crypto/dilithium_key.cpp"
            target="_blank"
            rel="noopener noreferrer"
          >
            crypto/dilithium_key.cpp
          </a>
          . The <code>dilithium_hd_key.cpp</code> file quoted in &ldquo;Why the Stub Breaks&rdquo; was scratch
          code that was never compiled into the wallet (it is absent from <code>src/Makefile.am</code>), even
          at v0.3.0.
        </p>
      </div>
      ...
      <section id="what-this-means">
        ...
        <div className="guide-note">
          <span className="guide-note-label">Update &middot; v0.4.1-testnet</span>
          <p>
            This no longer holds for hardened derivation. As of v0.4.1-testnet the wallet regenerates every
            Dilithium key deterministically from its HD seed, so a single seed backup recovers the wallet
            &mdash; you do not need to back up each key pair separately. What is still missing is watch-only
            (public-key-only) derivation; that part of the problem remains open.
          </p>
        </div>
