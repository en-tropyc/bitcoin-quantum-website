...
export default function MultiAlgorithmFutureGuide() {
  return (
    <GuideLayout
      ...
      slug="/guides/quantum-secure-bitcoin/multi-algorithm-future"
    ...
    >
      <section id="why-one-isnt-enough">
        ...
        <p>
          ...
          itself eliminated SIKE &mdash; a fourth-round KEM candidate &mdash; after a devastating classical
          attack was found in 2022, years into the evaluation process.
        </p>
        <p>
      ...
      <section id="framework-design">
        ...
        <div className="script-block">
          ...
          <div>&nbsp;&nbsp;NONE,&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// Placeholder &mdash; not yet read by validation</div>
        ...
        <p>
          This enum sketches the intended architecture for multi-algorithm support: a new scheme would be
          added by implementing its cryptographic primitives, adding the script opcodes that verify it, and
          gating activation through a consensus flag like this one &mdash; without redesigning the framework.
        </p>
        <div className="guide-note">
          <span className="guide-note-label">Correction &middot; v0.4.1-testnet</span>
          <p>
            An earlier draft described this field as if it were live &mdash; &ldquo;set to <code>NONE</code>
            (allowing any algorithm)&rdquo; and ready for &ldquo;soft-fork activation of algorithm
            enforcement.&rdquo; That overstates what ships. In BTQ through v0.4.1-testnet,
            <code>signature_algorithm</code> is an inert placeholder: it is initialized to <code>NONE</code> in
            every network&rsquo;s parameters and is never read by transaction or block validation. So
            <code>NONE</code> does not &ldquo;allow any algorithm,&rdquo; and changing it cannot by itself
            activate or enforce Falcon or SPHINCS+ &mdash; today the accepted algorithms are fixed entirely by
            which script opcodes are implemented.
          </p>
        </div>
        ...
        <p>
          For BIP-360&rsquo;s approach to Bitcoin, Pay-to-Merkle-Root (P2MR, originally drafted as P2QRH)
          reaches multi-algorithm support differently: the witness-v2 output commits only to a 32-byte Merkle
          root and carries no algorithm identifier &mdash; the choice of scheme lives in the leaf script that
          is revealed when the output is spent. Both designs converge on supporting multiple algorithms, which
          suggests multi-algorithm support is a natural requirement, not an over-engineering choice.
        </p>
      </section>
