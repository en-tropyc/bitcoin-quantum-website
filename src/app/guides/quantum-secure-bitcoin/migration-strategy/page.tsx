...
export default function MigrationStrategyGuide() {
  return (
    <GuideLayout
      ...
      slug="/guides/quantum-secure-bitcoin/migration-strategy"
    ...
    >
      ...
      <section id="dual-signature">
        ...
        <p>
          For Bitcoin, the equivalent would be a soft fork adding a new output type &mdash; BIP-360&rsquo;s
          Pay-to-Merkle-Root (P2MR), originally drafted as P2QRH &mdash; that existing nodes treat as
          &ldquo;anyone can spend&rdquo; while upgraded nodes enforce the new rules. In BIP-360&rsquo;s own
          scoping these are two complementary pieces: P2MR removes the exposed Taproot key path, and a
          separate signature/opcode change adds post-quantum verification; BTQ ships both together. Either
          way, it is the standard Bitcoin upgrade mechanism &mdash; the same approach used for SegWit and
          Taproot. Old transactions remain valid.
      ...
      <section id="exposed-keys">
        ...
        <p>
          ...
          key has already been exposed &mdash; through previous spends, the P2PK output format, or Taproot
          (P2TR) outputs, whose tweaked public key sits in the scriptPubKey from the moment the output is
          created, even if it has never been spent or reused. These funds are actively at quantum risk, and
          their owners may or may not be aware.
        </p>
        <p>
      ...
      <section id="hardware-wallets">
        ...
        <ul>
          ...
          <li>
            <strong>Cross-wallet HD compatibility</strong>: there is still no standardized, cross-wallet
            Dilithium HD scheme, so a seed backed up in one wallet may not restore in another. And because
            lattice keys admit no homomorphic public-key derivation, watch-only (extended-public-key)
            derivation is not available &mdash; Dilithium HD paths are hardened-only.
          </li>
        </ul>
        <div className="guide-note">
          <span className="guide-note-label">Update &middot; v0.4.1-testnet</span>
          <p>
            An earlier draft of this guide said seed phrases could not recover Dilithium keys and that users
            needed a separate backup per key. That reflected an earlier state of the codebase (the v0.3.x
            placeholder). As of v0.4.1-testnet, the wallet derives Dilithium keys deterministically from its HD
            seed via hardened-only paths that mirror <code>m/0&rsquo;/0&rsquo;/n&rsquo;</code>, so a single
            backed-up seed regenerates every key. What remains missing is watch-only (extended-public-key)
            derivation and a standardized cross-wallet scheme &mdash; see the{' '}
            <a href="/guides/quantum-secure-bitcoin/hd-wallet-derivation">HD Wallet Derivation</a> guide.
          </p>
        </div>
        <p>
